...
|---|---|---|---|
| `student` | агент, primary | Tab или агент по умолчанию | Держит принципы учебного режима, выбирает скилл |
...
1. Откройте OpenCode в папке своего проекта: `opencode`.
2. Нажмите Tab — в списке агентов есть `student`.
3. Наберите `/` — в списке команд есть `hint-ladder`, `code-review`, `debug-coach`, `explain-code`.
4. Напишите «я застрял» — агент загрузит `hint-ladder` и даст первую подсказку без кода.
